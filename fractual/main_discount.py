...
class Human(object):
    ...
    def __str__(self):
        ...
        if self.fx != 'death':
            return "age={:4},next_test={:5}, drug_end={:6}, last_hip={:4}, last_vf={:4}, last_wf={:4}, < inc_utils={:8} acc_utils={:8} | inc_cost={:8} acc_cost={:8} > |{:6}, {:6}|  fx={:6},     trt={:6},{:6},{:6},  ost_test={:6}, vfa_test={:6}, hip_cnt={:2}, vf_cnt={:2}, wf_cnt={:2}, trt_cnt={:4}, fx_rate={:6}".format(
                self.age, self.next_test, self.drug_end, self.last_hip, self.last_vf, self.last_wf, self.inc_utils, self.acc_utils, self.inc_cost, self.acc_cost, ost, vfa, self.fx, str(self.trt),str(self.trt_p1),str(self.trt_p2), self.ost_test, self.vfa_test, self.hip_cnt, self.vf_cnt, self.wf_cnt, self.trt_cnt, self.fx_rate)
        else:
            return "age={:4},next_test={:5}, drug_end={:6}, last_hip={:4}, last_vf={:4}, last_wf={:4}, < inc_utils={:8} acc_utils={:8} | inc_cost={:8} acc_cost={:8} > |{:6}, {:6}|  fx={:6}".format(
...
# OST_PREVAL[80] = 0.876
    ...
    def state_transition(self, prt=False):
        ...
        if prt:
            ret = "table {:15s}, old_fx={:5s},natural_death={:8f},no_fx={:8f},hip={:8f},vf={:8f},wf={:8f},fx_death={:8f}, new_fx={:5s}".format(self.get_status_str(), self.old_fx, natual_rate,prob_no_fix,prob_hip,prob_vf,prob_wf,prob_death, self.fx)
        # #TODO delete
        # print self.fx
        return ret
